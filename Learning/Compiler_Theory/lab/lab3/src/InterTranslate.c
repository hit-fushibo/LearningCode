...
void print_operand(Operand op, FILE *fp)
{
    if (op->kind == VARIABLE_OP)
        fprintf(fp, "v%d", op->op_u.var_no);
    else if (op->kind == CONSTANT_OP)
        fprintf(fp, "#%d", op->op_u.value);
    else if (op->kind == FUNCTION_OP)
        fprintf(fp, "%s", op->op_u.name);
    else if (op->kind == TEMP_OP)
        fprintf(fp, "t%d", op->op_u.var_no);
    else if (op->kind == LABEL_OP)
        fprintf(fp, "label%d", op->op_u.var_no);
}
...
void print_IR(FILE *fp)
{
    for (InterCodes c = inter_code_head; c != NULL; c = c->next)
    {
        ...
        else if (p->kind == FUNCTION_IR)
        { // FUNCTION f :
            fprintf(fp, "FUNCTION %s :\n", p->u.one.op->op_u.name);
        }
        else if (p->kind == ASSIGN_IR)
        { // op1 := op2
        ...
        else if (p->kind == GOTO_IR)
        { // GOTO x
            fprintf(fp, "GOTO ");
            print_operand(p->u.one.op, fp);
            fprintf(fp, "\n");
        }
        else if (p->kind == IFGOTO_IR)
        { // IF op1 [relop] op2 GOTO op3
            fprintf(fp, "IF ");
        ...
        else if (p->kind == RETURN_IR)
        { // RETURN x
            fprintf(fp, "RETURN ");
            print_operand(p->u.one.op, fp);
            fprintf(fp, "\n");
        }
        else if (p->kind == DEC_IR)
        { // DEC x [size]
            fprintf(fp, "DEC ");
            print_operand(p->u.dec.x, fp);
            fprintf(fp, " %d\n", p->u.dec.size);
        }
        else if (p->kind == ARG_IR)
        { // ARG x
            fprintf(fp, "ARG ");
            print_operand(p->u.one.op, fp);
            fprintf(fp, "\n");
        }
        else if (p->kind == CALL_IR)
        { // op1 := CALL op2,if(op1==NULL)call op2
            print_operand(p->u.two.op1, fp);
            fprintf(fp, " := ");
            fprintf(fp, "CALL ");
            print_operand(p->u.two.op2, fp);
        ...
        else if (p->kind == PARAM_IR)
        { // PARAM x
            fprintf(fp, "PARAM ");
            print_operand(p->u.one.op, fp);
            fprintf(fp, "\n");
        }
        else if (p->kind == READ_IR)
        { // READ x
            fprintf(fp, "READ ");
            print_operand(p->u.one.op, fp);
            fprintf(fp, "\n");
        }
        else if (p->kind == WRITE_IR)
        { // WRITE x
            fprintf(fp, "WRITE ");
            print_operand(p->u.one.op, fp);
            fprintf(fp, "\n");
...
int get_var_num(char *name)
{
    ...
    if (search == NULL)
    {
        printf("var %s not defined in symbol table!!!\n", name);
        assert(0);
    }
    if (search->var_num >= 0)
        return search->var_num;
    else
    {
        ++v_count;
...
int Addr_or_not(char *name)
{
    // 1 is a addr
    FieldList search = can_use_search(name, 1);
    if (search == NULL || search->addr_or_not != 1)
        return 0;
    else
        return 1;
}
...
int get_size(Type t)
{
    ...
    if (t->kind == BASIC)
        ...
        return t->u.array.size * get_size(t->u.array.elem);
    else if (t->kind == STRUCTURE)
    {
        int res = 0;
        for (FieldList p = t->u.structure; p != NULL; p = p->tail)
            res += get_size(p->type);
        return res;
    }
    else
...
void translate_ParamDec(struct Node *r)
{
    ...
    { // VarDec [INT]
        ...
        while (strcmp(child->Node_Type, "ID") != 0)
...
void translate_Dec(struct Node *r)
{
    ...
    if (r->first_child->sibling != NULL)
        exp = r->first_child->sibling->sibling;
    if (exp == NULL) // VarDec
...
void get_addr(struct Node *r, Operand place)
{
    ...
    {
        if (strcmp(r->first_child->sibling->Node_Type, "LB") == 0)
            get_addr_array(r, place);
        else if (strcmp(r->first_child->sibling->Node_Type, "DOT") == 0)
            get_addr_structure(r, place);
    }
}
...
Type get_type(struct Node *r)
{
    ...
    else if (strcmp(child->Node_Type, "LP") == 0)
        return sem_Exp(child->sibling);
...
void translate_Exp(struct Node *r, Operand place)
{
    ...
    if (strcmp(child->Node_Type, "Exp") == 0)
    {
        ...
        else if (strcmp(child->sibling->Node_Type, "AND") == 0 || strcmp(child->sibling->Node_Type, "OR") == 0 || strcmp(child->sibling->Node_Type, "RELOP") == 0)
        {
            Operand label1 = new_label();
            Operand label2 = new_label();
            //[place := #0]
        ...
        { //+-*/
            ...
            {
                InterCode c3 = (InterCode)malloc(sizeof(struct InterCode_));
                if (strcmp(exp_sym->Node_Type, "PLUS") == 0)
                    c3->kind = ADD_IR;
                else if (strcmp(exp_sym->Node_Type, "MINUS") == 0)
                    c3->kind = SUB_IR;
                else if (strcmp(exp_sym->Node_Type, "STAR") == 0)
                    c3->kind = MUL_IR;
                else if (strcmp(exp_sym->Node_Type, "DIV") == 0)
                    c3->kind = DIV_IR;
                else
                    print_tree(r, 0);
                c3->u.three.res = copy_operand(place);
        ...
        translate_Exp(child->sibling, place);
