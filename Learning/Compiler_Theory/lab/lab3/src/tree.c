...
void print_tree(struct Node* r,int pre) {
	...
	printf("%s", r->Node_Type);
	if (r->flex == 1) {
		if (strcmp(r->Node_Type, "ID") == 0)
			printf(": %s", r->content.my_char);
		else if (strcmp(r->Node_Type, "TYPE") == 0)
			printf(": %s", r->content.my_char);
		else if (strcmp(r->Node_Type, "INT") == 0)
			printf(": %d", r->content.my_int);
		else if (strcmp(r->Node_Type, "FLOAT") == 0)
