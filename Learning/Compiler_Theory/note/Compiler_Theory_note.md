...
- 在自底向上分析中总是采用最左规约的方式，所以最左规约也称规范规约。*编译器总是从左向右扫描输入的，所以最左规约是规范的：纯个人瞎bb*
...
- 从文法开始符号出发，在每一步推导过程中根据当前句型的最左非终结符A和当前输入符号a，选择正确的A-产生式。为了保证分析的确定性，选出的候选式必须是唯一的
...
- 自顶向下的语法采用最左推导方式、自底向上的语法分析采用最左规约方式
- 自底向上分析的通用框架

  - 移入-规约分析

  ![1710768748576](images/1710768748576.png)
- 栈内符号串+剩余输入=规范句型
- 每次规约的符号串称为句柄
- 分析栈中内容为某一规范句型的前缀
- 规范句型的活前缀
  ...
  - 不含句柄右侧任意符号的规范规范的前缀
  - LR自动机中从初始状态开始的每一条路径对应一个规范句型的活前缀

移入-规约分析的工作流程

- 首先将一个或者多个输入符号移入栈中，直到可以对栈中某个符号串进行规约为止
- 将待规约的符号串规约为某个产生式的左部
- 不断循环上面的流程，直到检测到一个语法错误，或者成功完成了语法分析（栈中包含了开始符号且输入缓冲区为空）

移入-规约分析器可采取的动作

- 移入：将输入符号压栈
- 规约：被规约的符号串的右端必然位于栈顶，语法分析器在栈中确定符号串的左端，并决定采用哪个非终结符替换这个串
- 接收：成功完成语法分析
...
- 规约-规约冲突
  ...
  ![1710769277202](images/1710769277202.png)

  - 这里语法分析器不知道是用第二个产生式还是第三个产生式进行规约
  - 造成错误的原因：错误地识别了句柄（句型的最左直接短语）
- 移入-规约冲突

  ![1710769381613](images/1710769381613.png)

### 4.4 LR分析法

LR文法

- 最大的、可以构造出相应的移入-规约语法分析器的文法类
  - L：对输入进行从左到右的扫描
  - R：反向构造一个最右推导序列
- LR(K)分析
  - 需要向前看k个输入符号的LR分析
  - k=0或1这两种情况具有实际意义，k缺省时为1

LR分析法的基本原理

- 用状态标识句柄的识别进度

  ![1710769562400](images/1710769562400.png)
- LR分析器基于这样的状态构造自动机进行句柄识别

LR分析器的总体结构

![1710769608356](images/1710769608356.png)

LR分析表的结构

![1710769631510](images/1710769631510.png)
...
![1710769714705](images/1710769714705.png)

![1710769775198](images/1710769775198.png)

![1710769783455](images/1710769783455.png)

LR分析算法

![1710769835876](images/1710769835876.png)
...
- 右部某位置标有圆点的产生式称为相应文法的一个LR(0)项目
  ...
  ![1710770158089](images/1710770158089.png)
- 项目描述了句柄识别的状态
- 产生式$A\rightarrow\epsilon$只产生一个项目$A\rightarrow\cdot$

增广文法

- 增加一条新的开始符号$S'$和产生式$S'\rightarrow S$得到的文法

  ![1710770368782](images/1710770368782.png)
- 引入新的开始产生式的目的是使得文法开始符号仅出现在一个产生式的左边，从而使得分析器只有一个接收状态

文法中的项目

![1710770552110](images/1710770552110.png)

- 后继项目
  - 同属一个产生式的项目，但圆点的位置只相差一个符号，则称后者为前者的后继项目
- 这些项目中存在等价项目，可以将等价的项目组成一个项目集(I)，称为项目集闭包，每个闭包对应自动机一个状态
  - 闭包计算

    ![1710770921907](images/1710770921907.png)

GOTO函数：构造每个项目集对应于文法符号X的后继项目集

![1710771052854](images/1710771052854.png)

构造LR(0)自动机项目集的算法

![1710771161626](images/1710771161626.png)

LR(0)分析表的构造方法

![1710771193208](images/1710771193208.png)

LR(0)依然会存在移进/规约冲突和规约/规约冲突

如果LR(0)分析表中没有语法分析动作冲突，那么给定的文法就称为LR(0)文法

不是所有的CFG都能用LR(0)方法进行更新，即CFG不总是LR(0)文法

#### 4.4.2 SLR分析

LR(0)自动机为什么消解不了某些冲突

* 句柄都是相对与一个句型而言的，因此应该将句柄的识别放在句型这样一个上下文环境中考虑
* 对于$A\rightarrow\beta$，是否应该将$\beta$规约为A取决于当前句型对应分析树中A所处的上下文环境
* LR(0)考虑了A的上文，但未考虑A的下文，因此消解冲突能力有限

SLR分析法的基本思想

![1716806996652](images/1716806996652.png)

SLR分析表构造算法

![1716807021330](images/1716807021330.png)
...
* 对于产生式$A\rightarrow\alpha$的规约，在不同的使用位置，A会要求不同的后继符号
* 在特定位置，A的后继符集合是FLLOW(A)的子集

规范LR(1)项目

* 将一般形式为$[A\rightarrow\alpha\cdot\beta,a]$的项称为LR(1)项，其中$A\rightarrow\alpha\beta$是一个产生式，a是一个终结符，他表示当前状态下A后必须紧跟的终结符，称为该项的展望符
  * 展望符只对待约项目起作用，表明只有下一个输入符号为a时才能利用该产生式规约
* 这样的a总是FOLLOW(A)的子集

等价LR(1)的项目

![1716885468244](images/1716885468244.png)

同心项目集：除了展望符之外两个LR(1)项目是相同的，则称这两个项目集为同心项目集

LR(1)项目集闭包

![1716885583764](images/1716885583764.png)

GOTO函数

![1716885596865](images/1716885596865.png)

为文法G构造LR(1)集族

![1716885659298](images/1716885659298.png)

LR分析表构造算法

![1716886003661](images/1716886003661.png)

#### 4.4.4 LALR分析

在LR(0)的基础上合并同心项目集

合并同心项目集时可能会出现规约规约冲突，但是不会出现移入规约冲突
...
* 分析能力介于SLR和LR(1)之间
  * LR(0)<SLR<LALR<LR(1)
* 合并后展望符集合仍未FOLLOW集的子集

#### 4.4.5 二义性文法的LR分析

通过人为规定运算优先级等在分析过程中消除二义性
...
* 恐慌模式错误恢复
  ![1716886357199](images/1716886357199.png)
* 短语层次错误恢复
  ![1716886383825](images/1716886383825.png)
...
![1716886508914](images/1716886508914.png)
...
* 是一种面向文法的翻译技术
* 语义信息的表示
  * 为CFG的文法符号设置语义属性，表示语法成分的语义信息
* 语义属性的计算
  * 用文法符号所在产生式相关联的语义规则计算
...
* 综合属性
  * 在分析树节点N上的非终结符A的综合属性只能通过N的子节点或N本身的属性定义
  * 终结符可以有综合属性，值是词法分析器提供的词法值
* 继承属性
  * 通过N的父节点或兄弟节点或本身定义
  * 终结符无继承属性
...
* 一个可行的求值顺序是依赖图的一个拓扑排序
* 对于只有综合属性的SDD，可以按照任何自底向上的顺序计算它们的值
* 对于同时拥有综合属性和继承属性的SDD，不能保证存在一个可行的求值顺序
...
![1716887413234](images/1716887413234.png)

### 5.3 语法制导翻译方案-SDT

SDT

* 在产生式右部中嵌入了程序片段(称为语义动作)的CFG
* 可以看作SDD的具体实施方案
  * SDD确定了怎么计算属性值
  * SDT进一步明确计算的时机
